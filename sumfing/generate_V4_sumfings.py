...
def settings(difficulty):
    ...
    return (min_tiles, max_tiles, min_answer, max_answer, operators, block_fractions, times_tables)
...
def generate_valid_sums(tiles, max_tiles, min_answer, max_answer, block_fractions, times_tables):
    ...
    if block_fractions == True:
        for result,expressions in selected_sums.items():
            solution = expressions[0]
            if check_for_fraction_step(solution) == True:
                selected_sums[result][0].remove()
                selected_sums[result].append(solution)

    # Check that there is an answer that is times_tables friendly
    #if times_tables == True:
    #       for result, expressions in selected_sums.items():
...
def generate_good_sums(level, tiles, min_tiles ,max_tiles, min_answer, max_answer, block_fractions, times_tables):
    ...
    valid_sums = generate_valid_sums(tiles, max_tiles,min_answer, max_answer, block_fractions, times_tables)
...
def generate_puzzle():
    ...
    for level in ["Easy", "Medium", "Hard"]:
        ...
        (min_tiles, max_tiles, min_answer, max_answer, operators, block_fractions, times_tables) = settings(level)
        tiles = numtiles + operators
        sums = generate_good_sums(level, tiles, min_tiles, max_tiles, min_answer, max_answer, block_fractions, times_tables)
